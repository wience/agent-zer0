...
import { CalendarIcon, MailIcon, MicIcon, UserIcon } from "lucide-react";
...
const DATA = {
  navbar: [
    ...
    { href: "/agent", icon: UserIcon, label: "Send to Human Agent" },
  ],
  contact: {
    social: {
      GitHub: {
...
export function AppDock() {
  return (
    <div className="fixed bottom-4 z-[100] left-1/2 transform -translate-x-1/2">
      <TooltipProvider>
        <Dock className="bg-white" direction="middle">
          ...
          {Object.entries(DATA.contact.social).map(([name, social]) => (
            <DockIcon key={name}>
              <Tooltip>
                <TooltipTrigger asChild>
                  {!(social as any).shareButton ? <Link
                    href={social.url}
